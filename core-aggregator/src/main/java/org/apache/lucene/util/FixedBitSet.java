...
/**
 * BitSet of fixed length (numBits), backed by accessible ({@link #getBits}) long[], accessed with an int index, implementing {@link Bits} and {@link DocIdSet}.
 * If you need to manage more than 2.1B bits, use {@link LongBitSet}.
 *
 * @lucene.internal
 */
public final class FixedBitSet extends BitSet implements MutableBits, Accountable {
    ...
    /**
     * If the given {@link FixedBitSet} is large enough to hold {@code numBits+1}, returns the given bits, otherwise returns a new {@link FixedBitSet} which can
     * hold the requested number of bits.
     * <p>
     * <b>NOTE:</b> the returned bitset reuses the underlying {@code long[]} of
     * the given {@code bits} if possible. Also, calling {@link #length()} on the returned bits may return a value greater than {@code numBits}.
     */
    ...
    /**
     * Creates a new LongBitSet using the provided long[] array as backing store. The storedBits array must be large enough to accommodate the numBits
     * specified, but may be larger. In that case the 'extra' or 'ghost' bits must be clear (or they may provoke spurious side-effects)
     *
    ...
    /**
     * Scans the backing store to check if all bits are clear. The method is deliberately not called "isEmpty" to emphasize it is not low cost (as isEmpty
     * usually is).
     *
