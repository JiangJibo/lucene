...
public final class IntBlockPool {
     ...
     *  intUpto + intOffset 代表当前数据在buffers里的位置
     */
    ...
    public void nextBuffer() {
        // 如果buffers里元素个数已经满了(从0开始),扩容成原来的1.5倍长度
        if (1 + bufferUpto == buffers.length) {
            int[][] newBuffers = new int[(int)(buffers.length * 1.5)][];
            System.arraycopy(buffers, 0, newBuffers, 0, buffers.length);
            buffers = newBuffers;
        }
        // 每次增加一个int[8192]的数组用来做buffer, 放在buffers里,buffers的长度默认是10
        buffer = buffers[1 + bufferUpto] = allocator.getIntBlock();
        // buffers里当前buffer的序号
        bufferUpto++;
        // 重置buffer里数据的使用offset
        intUpto = 0;
        // 设置总的buffers里的int的可用空间, 每增加一个buffer, +INT_BLOCK_SIZE
        intOffset += INT_BLOCK_SIZE;
    }
