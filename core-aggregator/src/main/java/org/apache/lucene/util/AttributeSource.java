...
public class AttributeSource {
    ...
    private static final ClassValue<Class<? extends Attribute>[]> implInterfaces = new ClassValue<Class<? extends Attribute>[]>() {
        ...
        protected Class<? extends Attribute>[] computeValue(Class<?> clazz) {
            ...
            final Class<? extends Attribute>[] a =
    ...
    public final void addAttributeImpl(final AttributeImpl att) {
        ...
        // 找到 PackedTokenAttributeImpl 的所有父接口 ：TypeAttribute, PositionIncrementAttribute,PositionLengthAttribute, OffsetAttribute, TermFrequencyAttribute
        for (final Class<? extends Attribute> curInterface : getAttributeInterfaces(clazz)) {
            // Attribute is a superclass of this interface
            if (!attributes.containsKey(curInterface)) {
                // invalidate state to force recomputation in captureState()
                this.currentState[0] = null;
                // 将所有的attr的实现都设置为PackedTokenAttributeImpl
    ...
    public final <T extends Attribute> T addAttribute(Class<T> attClass) {
        ...
        if (attImpl == null) {
            ...
            // 给 PackedTokenAttributeImpl 里添加各种内置属性实现,比如Position,Offset等
     ...
     *
     * @see PackedTokenAttributeImpl#positionIncrement
     * @see PackedTokenAttributeImpl#positionLength;
     * @see PackedTokenAttributeImpl#startOffset;
     * @see PackedTokenAttributeImpl#endOffset;
     * @see PackedTokenAttributeImpl#type;
     * @see PackedTokenAttributeImpl#termFrequency;
    ...
    /**
     * 将之前存好的属性恢复给当前数据, 比如我在解析一个term时,此term会被特殊处,需要用上一个term的数据,那么在处理上一个是调用 {@link #captureState}
     * 在此term时{@link #restoreState(State)} 就能拿到上一个term的所有信息
