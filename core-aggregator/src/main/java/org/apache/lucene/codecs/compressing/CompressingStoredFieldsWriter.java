...
public final class CompressingStoredFieldsWriter extends StoredFieldsWriter {
    ...
    // lucene的数据结构，每个Field存储的都是下列类型中国男的一个
    ...
    public CompressingStoredFieldsWriter(Directory directory, SegmentInfo si, String segmentSuffix, IOContext context,
        String formatName, CompressionMode compressionMode, int chunkSize, int maxDocsPerChunk, int blockSize)
        throws IOException {
    ...
    private void writeHeader(int docBase, int numBufferedDocs, int[] numStoredFields, int[] lengths, boolean sliced)
        throws IOException {
    ...
    private void flush() throws IOException {
        ...
        if (sliced) {
            ...
            for (int compressed = 0; compressed < bufferedDocs.getPosition(); compressed += chunkSize) {
                compressor.compress(bufferedDocs.getBytes(), compressed,
                    Math.min(chunkSize, bufferedDocs.getPosition() - compressed), fieldsStream);
            }
        } else {
    ...
    public int merge(MergeState mergeState) throws IOException {
        ...
        if (mergeState.needsIndexSort) {
            ...
            for (int i = 0; i < mergeState.storedFieldsReaders.length; i++) {
                if (matching.matchingReaders[i] &&
                    ...
                    subs.add(new CompressingStoredFieldsMergeSub(storedFieldsReader, mergeState.docMaps[i],
        ...
        for (int readerIndex = 0; readerIndex < numReaders; readerIndex++) {
            ...
            if (matchingFieldsReader == null || matchingFieldsReader.getVersion() != VERSION_CURRENT
                ...
                // read the docstart + doccount from the chunk header (we write a new header, since doc numbers will
              // change),
                ...
                while (docID < maxDoc) {
                    ...
                    if (docID > maxDoc) {
                        throw new CorruptIndexException(
                            "invalid state: base=" + base + ", count=" + bufferedDocs + ", maxDoc=" + maxDoc, rawDocs);
                    }
                ...
                if (rawDocs.getFilePointer() != matchingFieldsReader.getMaxPointer()) {
                    throw new CorruptIndexException(
                        "invalid state: pos=" + rawDocs.getFilePointer() + ", max=" + matchingFieldsReader
                            .getMaxPointer(), rawDocs);
                }
    ...
    private static class CompressingStoredFieldsMergeSub extends DocIDMerger.Sub {
        ...
        public CompressingStoredFieldsMergeSub(CompressingStoredFieldsReader reader, MergeState.DocMap docMap,
            int maxDoc) {
            super(docMap);
            this.maxDoc = maxDoc;
            this.reader = reader;
