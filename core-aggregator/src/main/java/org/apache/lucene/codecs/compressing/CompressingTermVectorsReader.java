...
public final class CompressingTermVectorsReader extends TermVectorsReader implements Closeable {
    ...
    public CompressingTermVectorsReader(Directory d, SegmentInfo si, String segmentSuffix, FieldInfos fn,
        ...
        try (ChecksumIndexInput input = d.openChecksumInput(indexName, context)) {
            ...
            try {
                ...
                version = CodecUtil.checkIndexHeader(input, codecNameIdx, VERSION_START, VERSION_CURRENT, si.getId(),
                    segmentSuffix);
        ...
        try {
            ...
            int version2 = CodecUtil.checkIndexHeader(vectorsStream, codecNameDat, VERSION_START, VERSION_CURRENT,
            ...
            if (numDirtyChunks > numChunks) {
                throw new CorruptIndexException(
                    "invalid chunk counts: dirty=" + numDirtyChunks + ", total=" + numChunks, vectorsStream);
            }
    ...
    public Fields get(int doc) throws IOException {
        ...
        final int[] fieldNums;
        {
            ...
            ++totalDistinctFields;
            final PackedInts.ReaderIterator it = PackedInts.getReaderIteratorNoHeader(vectorsStream,
        ...
        final PackedInts.Reader flags;
        {
            ...
            final PackedInts.Reader allFieldNumOffs = PackedInts.getReaderNoHeader(vectorsStream,
                PackedInts.Format.PACKED, packedIntsVersion, totalFields, bitsPerOff);
            switch (vectorsStream.readVInt()) {
                case 0:
                    final PackedInts.Reader fieldFlags = PackedInts.getReaderNoHeader(vectorsStream,
                    ...
                    break;
                case 1:
                    flags = PackedInts.getReaderNoHeader(vectorsStream, PackedInts.Format.PACKED, packedIntsVersion,
        ...
        final int totalTerms;
        {
            ...
            numTerms = PackedInts.getReaderNoHeader(vectorsStream, PackedInts.Format.PACKED, packedIntsVersion,
        ...
        if (totalPositions > 0) {
            positions = readPositions(skip, numFields, flags, numTerms, termFreqs, POSITIONS, totalPositions,
                positionIndex);
        } else {
        ...
        if (totalOffsets > 0) {
            ...
            startOffsets = readPositions(skip, numFields, flags, numTerms, termFreqs, OFFSETS, totalOffsets,
                positionIndex);
        ...
        decompressor.decompress(vectorsStream, totalLen + totalPayloadLength, docOff + payloadOff, docLen + payloadLen,
            suffixBytes);
    ...
    private int[][] readPositions(int skip, int numFields, PackedInts.Reader flags, PackedInts.Reader numTerms,
        int[] termFreqs, int flag, final int totalPositions, int[][] positionIndex) throws IOException {
    ...
    private static class TVTerms extends Terms {
        ...
        public TermsEnum iterator() throws IOException {
            ...
            termsEnum.reset(numTerms, flags, prefixLengths, suffixLengths, termFreqs, positionIndex, positions,
    ...
    private static class TVTermsEnum extends TermsEnum {
        ...
        private int[] prefixLengths, suffixLengths, termFreqs, positionIndex, positions, startOffsets, lengths,
            payloadIndex;
        ...
        void reset(int numTerms, int flags, int[] prefixLengths, int[] suffixLengths, int[] termFreqs,
            int[] positionIndex, int[] positions, int[] startOffsets, int[] lengths,
        ...
        public final PostingsEnum postings(PostingsEnum reuse, int flags) throws IOException {
            ...
            docsEnum.reset(termFreqs[ord], positionIndex[ord], positions, startOffsets, lengths, payloads,
                payloadIndex);
            return docsEnum;
        }
