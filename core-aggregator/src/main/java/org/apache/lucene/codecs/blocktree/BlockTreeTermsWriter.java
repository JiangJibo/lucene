 ...
 * <li>OuterNode --&gt; EntryCount, SuffixLength, Byte<sup>SuffixLength</sup>, StatsLength, &lt; TermStats &gt;<sup>EntryCount</sup>, MetaLength,
...
public final class BlockTreeTermsWriter extends FieldsConsumer {
    ...
    final int maxDoc;
    /**
     * @see BlockTreeTermsWriter#DEFAULT_MIN_BLOCK_SIZE
     */
    final int minItemsInBlock;
    /**
     * @see BlockTreeTermsWriter#DEFAULT_MAX_BLOCK_SIZE
     */
    ...
    public static void validateSettings(int minItemsInBlock, int maxItemsInBlock) {
        ...
        if (2 * (minItemsInBlock - 1) > maxItemsInBlock) {
            throw new IllegalArgumentException(
                "maxItemsInBlock must be at least 2*(minItemsInBlock-1); got maxItemsInBlock=" + maxItemsInBlock + " minItemsInBlock=" + minItemsInBlock);
        }
    }
    ...
    class TermsWriter {
        ...
        /**
         * prefixStarts.length 》= 最近处理过的term的最大长度, prefixStarts[N]=M 表示,表示第M个term有N长度,之前的term都没有这个长度
         * 比如第一个term: "java" ，则 prefixStarts[0] = 1, prefixStarts[1] = 1, prefixStarts[2] = 1, prefixStarts[3] = 1,
         */
         ...
         *
         * @param prefixLength 从 {@link #prefixStarts}的第几个元素开始, 也就是实例化这个位置的term的长度
         * @param count        写入多少个
         * @throws IOException
         */
            ...
            for (int i = start; i < end; i++) {
                ...
                if (ent.isTerm) {
                    ...
                    if (term.termBytes.length == prefixLength) {
                        ...
                        // 后缀起始字符为: term.termBytes[prefixLength] & 1111 1111
                 ...
                 if (true){
                     System.out.println("  i=" + i + " ent=" + ent + " suffixLeadLabel=" + suffixLeadLabel);
                 }

                if (suffixLeadLabel != lastSuffixLeadLabel) {
                    int itemsInBlock = i - nextBlockStart;
                    // 如果当前位置距离第一个被写入的term的序号 > 25 || 当前term距离结束term之间超过48个term
        ...
        private PendingBlock writeBlock(int prefixLength, boolean isFloor, int floorLeadLabel, int start, int end,
            ...
            //if (DEBUG2) System.out.println("    writeBlock field=" + fieldInfo.name + " prefix=" + brToString(prefix) + " fp=" + startFP + " isFloor=" +
            // isFloor + " isLastInFloor=" + (end == pending.size()) + " floorLeadLabel=" + floorLeadLabel + " start=" + start + " end=" + end + " hasTerms=" +
      ...
      /*
      if (DEBUG) {
        System.out.println("  writeBlock " + (isFloor ? "(floor) " : "") + "seg=" + segment + " pending.size()=" + pending.size() + " prefixLength=" +
        prefixLength + " indexPrefix=" + brToString(prefix) + " entCount=" + (end-start+1) + " startFP=" + startFP + (isFloor ? (" floorLeadLabel=" + Integer
        .toHexString(floorLeadLabel)) : ""));
      }
      */
            ...
            if (isLeafBlock) {
                ...
                for (int i = start; i < end; i++) {
                    ...
                    if (ent.isTerm) {
                        ...
                        //  suffixBytes.length = suffix;
                        //  System.out.println("      write sub-block suffix=" + brToString(suffixBytes) + " subFP=" + block.fp + " subCode=" +
                        //  (startFP-block.fp) + " floor=" + block.isFloor);
                        //}

                        assert floorLeadLabel == -1 || (block.prefix.bytes[prefixLength] & 0xff) >= floorLeadLabel : "floorLeadLabel=" + floorLeadLabel
        ...
        private void pushTerm(BytesRef text) throws IOException {
            ...
            if (true) {
                System.out.println("  shared=" + pos + "  lastTerm.length=" + lastTerm.length());
            }

            // 如果上一个term是jiangjibo, 当前jibo， 则pos = 2 , lastTerm.length()=9, 也就是处理 8-2 之间共享前缀数据， 也就是处理上一个term和当前term之间不同的字符信息
            // Close the "abandoned" suffix now: 如果当前的前缀池已经不起作用了，关闭它，截止pos位置开始, 也就是当前共享前缀的长度
            for (int i = lastTerm.length() - 1; i >= pos; i--) {

                // How many items on top of the stack share the current suffix
                // we are closing:
                // 初始化这个位置的term距离现在最新的term, 他们间相差多少term
                int prefixTopSize = pending.size() - prefixStarts[i];

                // prefixTopSize >= 25 , 距离这个term已经超过了25个term
