...
final class IndexFileDeleter implements Closeable {
     ...
     * @param directoryOrig
     * @param directory
     * @param policy             {@link KeepOnlyLastCommitDeletionPolicy}
     * @param segmentInfos       最近一个segments文件生成的信息
     * @param infoStream
     * @param writer
     * @param initialIndexExists
     * @param isReaderInit
    ...
    public IndexFileDeleter(String[] files, Directory directoryOrig, Directory directory, IndexDeletionPolicy policy, SegmentInfos segmentInfos,
        ...
        if (currentSegmentsFile != null) {
            ...
            // 读取每个segment文件,生成提交点CommitPoint
            for (String fileName : files) {
                m.reset(fileName);
                // 只处理setmengs_N 的文件
                if (!fileName.endsWith("write.lock") && (m.matches() || fileName.startsWith(IndexFileNames.SEGMENTS) || fileName.startsWith(
                    ...
                    if (fileName.startsWith(IndexFileNames.SEGMENTS) && !fileName.equals(IndexFileNames.OLD_SEGMENTS_GEN)) {
                        ...
                        // 每个segment文件对应一个CommitPoint, 提交点
        ...
        // startup:  KeepOnlyLastCommitDeletionPolicy.onInit, 在开始的时候就删除所有的segments文件,只保留最近的一个
    ...
    static void inflateGens(SegmentInfos infos, Collection<String> files, InfoStream infoStream) {
        ...
        for (SegmentCommitInfo info : infos) {
            ...
            if (info.getNextWriteDelGen() < genLong + 1) {
                if (infoStream.isEnabled("IFD")) {
                    infoStream.message("IFD",
                        "init: seg=" + info.info.name + " set nextWriteDelGen=" + (genLong + 1) + " vs current=" + info.getNextWriteDelGen());
                }
            ...
            if (info.getNextWriteFieldInfosGen() < genLong + 1) {
                if (infoStream.isEnabled("IFD")) {
                    infoStream.message("IFD",
                        "init: seg=" + info.info.name + " set nextWriteFieldInfosGen=" + (genLong + 1) + " vs current=" + info.getNextWriteFieldInfosGen());
                }
            ...
            if (info.getNextWriteDocValuesGen() < genLong + 1) {
                if (infoStream.isEnabled("IFD")) {
                    infoStream.message("IFD",
                        "init: seg=" + info.info.name + " set nextWriteDocValuesGen=" + (genLong + 1) + " vs current=" + info.getNextWriteDocValuesGen());
                }
    ...
    public void checkpoint(SegmentInfos segmentInfos, boolean isCommit) throws IOException {
        ...
        if (infoStream.isEnabled("IFD")) {
            infoStream.message("IFD",
                "now checkpoint \"" + writer.segString(writer.toLiveInfos(segmentInfos)) + "\" [" + segmentInfos.size() + " segments " + "; isCommit = "
                    + isCommit + "]");
        }
        ...
        if (isCommit) {
            ...
            // Tell policy so it can remove commits: 将之前的所有的commit加入到commitsToDelete
            policy.onCommit(commits);
