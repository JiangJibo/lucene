    ...
    private MergeSpecification findForcedMergesSizeLimit(
        ...
        while (start >= 0) {
            ...
            if (size(info, writer) > maxMergeSizeForForcedMerge || sizeDocs(info, writer) > maxMergeDocs) {
                if (verbose(writer)) {
                    message("findForcedMergesSizeLimit: skip segment=" + info + ": size is > maxMergeSize (" + maxMergeSizeForForcedMerge
                        + ") or sizeDocs is > maxMergeDocs (" + maxMergeDocs + ")", writer);
                }
    ...
    public MergeSpecification findForcedDeletesMerges(SegmentInfos segmentInfos, IndexWriter writer)
        ...
        for (int i = 0; i < numSegments; i++) {
            ...
            if (delCount > 0) {
                ...
                if (firstSegmentWithDeletions == -1) { firstSegmentWithDeletions = i; } else if (i - firstSegmentWithDeletions == mergeFactor) {
    ...
    public MergeSpecification findMerges(MergeTrigger mergeTrigger, SegmentInfos infos, IndexWriter writer) throws IOException {
        ...
        for (int i = 0; i < numSegments; i++) {
            ...
            if (verbose(writer)) {
                ...
                message(
                    "seg=" + writer.segString(info) + " level=" + infoLevel.level + " size=" + String.format(Locale.ROOT, "%.3f MB", segBytes / 1024 / 1024.)
                        + extra, writer);
            }
        }

        final float levelFloor;
        if (minMergeSize <= 0) { levelFloor = (float)0.0; } else { levelFloor = (float)(Math.log(minMergeSize) / norm); }
