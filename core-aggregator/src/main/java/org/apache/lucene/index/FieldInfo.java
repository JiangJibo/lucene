...
public final class FieldInfo {
     ...
     * @param name                属性名称
     * @param number              属性序号
     * @param storeTermVector     是否存储单词矢量
     * @param omitNorms           是否忽略Norms
     * @param storePayloads       是否存储载荷
     * @param indexOptions        IndexOptions类型
     * @param docValues           DocValues类型
     * @param dvGen
     * @param attributes          额外属性
     * @param pointDimensionCount
     * @param pointNumBytes
     */
    ...
    void update(boolean storeTermVector, boolean omitNorms, boolean storePayloads, IndexOptions indexOptions,
        ...
        if (this.pointDimensionCount == 0 && dimensionCount != 0) {
            ...
            throw new IllegalArgumentException(
                "cannot change field \"" + name + "\" from points dimensionCount=" + this.pointDimensionCount + ", numBytes=" + this.pointNumBytes
                    + " to inconsistent dimensionCount=" + dimensionCount + ", numBytes=" + dimensionNumBytes);
        }
    ...
    public void setPointDimensions(int count, int numBytes) {
        ...
        if (count > PointValues.MAX_DIMENSIONS) {
            throw new IllegalArgumentException(
                "point dimension count must be < PointValues.MAX_DIMENSIONS (= " + PointValues.MAX_DIMENSIONS + "); got " + count + " for field=\"" + name
                    + "\"");
        }
        if (numBytes <= 0) {
        ...
        if (numBytes > PointValues.MAX_NUM_BYTES) {
            throw new IllegalArgumentException(
                "point numBytes must be <= PointValues.MAX_NUM_BYTES (= " + PointValues.MAX_NUM_BYTES + "); got " + numBytes + " for field=\"" + name + "\"");
        }
        if (pointDimensionCount != 0 && pointDimensionCount != count) {
            throw new IllegalArgumentException(
                "cannot change point dimension count from " + pointDimensionCount + " to " + count + " for field=\"" + name + "\"");
        }
