...
public final class ReaderUtil {
    ...
    /**
     * 根据docID定位其位于哪个segment
     * Returns index of the searcher/reader for document <code>n</code> in the
     * array used to construct this searcher/reader.
     *
     * @param n         全局docID
     * @param docStarts 每个segment的第一个doc的全局ID
     * @return
     */
    public static int subIndex(int n, int[] docStarts) { // find
        ...
        while (hi >= lo) {
            // 二分查找法,  从最中间的segment开始找,
            int mid = (lo + hi) >>> 1;
            // 中间的segment的 firstDocumentGlobalId
    ...
    public static int subIndex(int n, List<LeafReaderContext> leaves) { // find
        ...
        while (hi >= lo) {
            ...
            if (n < midValue) { hi = mid - 1; } else if (n > midValue) { lo = mid + 1; } else { // found a match
