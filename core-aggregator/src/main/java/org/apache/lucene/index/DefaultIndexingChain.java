...
final class DefaultIndexingChain extends DocConsumer {
    ...
    // 1.7.0_25:  存储Field Name的数组,每个元素又有next,类似HashMap
                    ...
                    if (perField.pointValuesWriter != null) {
                        if (perField.fieldInfo.getPointDimensionCount() == 0) {
                            ...
                            throw new AssertionError(
                                "segment=" + state.segmentInfo + ": field=\"" + perField.fieldInfo.name + "\" has no points but wrote them");
                        }
                        if (pointsWriter == null) {
                            ...
                            if (fmt == null) {
                                throw new IllegalStateException(
                                    "field=\"" + perField.fieldInfo.name + "\" was indexed as points but codec does not support points");
                            }
                        ...
                        // BUG
                        throw new AssertionError(
                            "segment=" + state.segmentInfo + ": field=\"" + perField.fieldInfo.name + "\" has points but did not write them");
                    }
                    perField = perField.next;
                }
            }
            ...
            for (int i = 0; i < fieldHash.length; i++) {
                ...
                while (perField != null) {
                    if (perField.docValuesWriter != null) {
                        ...
                        // BUG
                        throw new AssertionError(
                            "segment=" + state.segmentInfo + ": field=\"" + perField.fieldInfo.name + "\" has docValues but did not write them");
                    }
                    perField = perField.next;
                }
            }
    ...
    private PerField getOrAddField(String name, IndexableFieldType fieldType, boolean invert) {
        ...
        final int hashPos = name.hashCode() & hashMask;
        PerField fp = fieldHash[hashPos];
        // 在fieldHash上找到这个Field对应的PerField
        while (fp != null && !fp.fieldInfo.name.equals(name)) {
            fp = fp.next;
        }
        // 如果这个Field的值是第一次存入,也就是之前是定了此Field,但一直没有值写入
        if (fp == null) {
            ...
            // At most 50% load factor: 当元素数量达到数组长度的一半时，rehash,参考HashMap
    ...
    private final class PerField implements Comparable<PerField> {
        ...
        public void invert(IndexableField field, boolean first) throws IOException, AbortingException {
            ...
            // field.tokenStream : 根据field的数据类型,用指定的分词器去拆分，得到拆分结果
                    ...
                    if (invertState.position < invertState.lastPosition) {
                        if (posIncr == 0) {
                            ...
                            throw new IllegalArgumentException(
                                "position overflowed Integer.MAX_VALUE (got posIncr=" + posIncr + " lastPosition=" + invertState.lastPosition + " position="
                                    + invertState.position + ") for field '" + field.name() + "'");
                        }
