...
class ReadersAndUpdates {
    ...
    public ReadersAndUpdates(IndexWriter writer, SegmentReader reader) {
        ...
        assert pendingDeleteCount >= 0 : "got " + pendingDeleteCount + " reader.numDeletedDocs()=" + reader
            .numDeletedDocs() + " info.getDelCount()=" + info.getDelCount() + " maxDoc=" + reader.maxDoc() + " numDocs="
            + reader.numDocs();
    }
    ...
    private synchronized boolean assertNoDupGen(List<DocValuesFieldUpdates> fieldUpdates,
    ...
    /**
     * Adds a new resolved (meaning it maps docIDs to new values) doc values packet.  We buffer these in RAM and write
     * to disk when too much
    ...
    public synchronized boolean verifyDocCounts() {
        ...
        assert info.info.maxDoc() - info.getDelCount() - pendingDeleteCount == count : "info.maxDoc=" + info.info
            .maxDoc() + " info.getDelCount()=" + info.getDelCount() + " pendingDeleteCount=" + pendingDeleteCount
            + " count=" + count;
        return true;
    }
    ...
    /**
     * setment删除doc, 线程同步方法, 多个dwpt会同时调用删除操作,保证每个segment里删除docID的正确性
     *
     * @param docID
     * @return
     * @throws IOException
    ...
    public synchronized boolean delete(int docID) throws IOException {
        ...
        assert docID >= 0 && docID < liveDocs.length() : "out of bounds: docid=" + docID + " liveDocsLength=" + liveDocs
            .length() + " seg=" + info.info.name + " maxDoc=" + info.info.maxDoc();
    ...
    public synchronized SegmentReader getReadOnlyClone(IOContext context) throws IOException {
        ...
        if (liveDocs != null) {
            return new SegmentReader(reader.getSegmentInfo(), reader, liveDocs,
                info.info.maxDoc() - info.getDelCount() - pendingDeleteCount);
        } else {
    ...
    private synchronized void handleNumericDVUpdates(FieldInfos infos,
        ...
        for (Entry<String, List<DocValuesFieldUpdates>> ent : pendingDVUpdates.entrySet()) {
            ...
            final SegmentWriteState state = new SegmentWriteState(null, trackingDir, info.info, fieldInfos, null,
                updatesContext, segmentSuffix);
            try (final DocValuesConsumer fieldsConsumer = dvFormat.fieldsConsumer(state)) {
                ...
                fieldsConsumer.addNumericField(fieldInfo, new EmptyDocValuesProducer() {
                    ...
                    public NumericDocValues getNumeric(FieldInfo fieldInfoIn) throws IOException {
                        ...
                        DocValuesFieldUpdates.Iterator[] subs = new DocValuesFieldUpdates.Iterator[updatesToApply
                            .size()];
    ...
    public synchronized boolean writeFieldUpdates(Directory dir, long maxDelGen, InfoStream infoStream)
        throws IOException {
        ...
        try {
            ...
            try {
                ...
                handleNumericDVUpdates(fieldInfos, trackingDir, docValuesFormat, reader, newDVFiles, maxDelGen,
                    infoStream);
                handleBinaryDVUpdates(fieldInfos, trackingDir, docValuesFormat, reader, newDVFiles, maxDelGen,
                    infoStream);

                fieldInfosFiles = writeFieldInfosGen(fieldInfos, trackingDir, docValuesFormat,
        ...
        if (infoStream.isEnabled("BD")) {
            infoStream.message("BD",
                String.format(Locale.ROOT, "done write field updates for seg=%s; took %.3fs; new files: %s",
    ...
    synchronized SegmentReader getReaderForMerge(IOContext context) throws IOException {
        ...
        if (delCount != reader.numDeletedDocs()) {
            ...
            // beware of zombies:
            assert delCount > reader.numDeletedDocs() : "delCount=" + delCount + " reader.numDeletedDocs()=" + reader
                .numDeletedDocs();
    ...
    public synchronized Map<String, List<DocValuesFieldUpdates>> getMergingDVUpdates() {
        // We must atomically (in single sync'd block) clear isMerging when we return the DV updates otherwise we can
        // lose updates:
        isMerging = false;
