...
public abstract class ReferenceManager<G> implements Closeable {
        ...
        /* if we can't increment the reader but we are
           ...
           the reference. */
                throw new IllegalStateException(
                    "The managed reference has already closed - this is likely a bug when the reference count is "
                        + "modified outside of the ReferenceManager");
            }
        } while (true);
    }
    ...
    /**
     * 执行刷新, 这样OS Cache 里的segment数据才能马上被探查到, 能被马上使用, 而不需要等待其写入磁盘
