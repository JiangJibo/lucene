...
/**
 * A Query that matches documents containing a particular sequence of terms. A PhraseQuery is built by QueryParser for input like <code>"new york"</code>.
 *
 * <p>This query may be combined with other terms or queries with a {@link BooleanQuery}.
 *
 * <p><b>NOTE</b>:
 * All terms in the phrase must match, even those at the same position. If you have terms at the same position, perhaps synonyms, you probably want {@link
 * MultiPhraseQuery} instead which only requires one term at a position to match.
...
public class PhraseQuery extends Query {
    ...
    public static class Builder {
        ...
        /**
         * Adds a term to the end of the query phrase. The relative position of the term within the phrase is specified explicitly, but must be greater than or
         * equal to that of the previously added term. A greater position allows phrases with gaps (e.g. in connection with stopwords). If the position is
         * equal, you most likely should be using {@link MultiPhraseQuery} instead which only requires one term at each position to match; this class requires
         * all of them.
         */
    ...
    /**
     * Create a phrase query which will match documents that contain the given list of terms at consecutive positions in {@code field}, and at a maximum edit
     * distance of {@code slop}. For more complicated use-cases, use {@link PhraseQuery.Builder}.
     *
     * @see #getSlop()
     */
    ...
    /**
     * Create a phrase query which will match documents that contain the given list of terms at consecutive positions in {@code field}.
     */
     ...
     * defined in this {@link PhraseQuery} and the positions of terms in a document.
     *
     * <p>For instance, when searching for {@code "quick fox"}, it is expected that
     * the difference between the positions of {@code fox} and {@code quick} is 1. So {@code "a quick brown fox"} would be at an edit distance of 1 since the
     * difference of the positions of {@code fox} and {@code quick} is 2. Similarly, {@code "the fox is quick"} would be at an edit distance of 3 since the
     * difference of the positions of {@code fox} and {@code quick} is -2. The slop defines the maximum edit distance for a document to match.
     *
        ...
        public Scorer scorer(LeafReaderContext context) throws IOException {
            ...
            if (fieldTerms.hasPositions() == false) {
                throw new IllegalStateException(
                    "field \"" + field + "\" was indexed without position data; cannot run PhraseQuery (phrase=" + getQuery() + ")");
            }
    ...
    /**
     * A guess of the average number of simple operations for the initial seek and buffer refill per document for the positions of a term. See also {@link
     * Lucene50PostingsReader.BlockPostingsEnum#nextPosition()}.
     * <p>
     * Aside: Instead of being constant this could depend among others on {@link Lucene50PostingsFormat#BLOCK_SIZE}, {@link TermsEnum#docFreq()}, {@link
     * TermsEnum#totalTermFreq()}, {@link DocIdSetIterator#cost()} (expected number of matching docs), {@link LeafReader#maxDoc()} (total number of docs in the
     * segment), and the seek time and block size of the device storing the index.
     */
    private static final int TERM_POSNS_SEEK_OPS_PER_DOC = 128;

    /**
     * Number of simple operations in {@link Lucene50PostingsReader.BlockPostingsEnum#nextPosition()} when no seek or buffer refill is done.
     */
    private static final int TERM_OPS_PER_POS = 7;

    /**
     * Returns an expected cost in simple operations of processing the occurrences of a term in a document that contains the term. This is for use by {@link
     * TwoPhaseIterator#matchCost} implementations.
