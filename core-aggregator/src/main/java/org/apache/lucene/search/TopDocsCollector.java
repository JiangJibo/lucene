...
public abstract class TopDocsCollector<T extends ScoreDoc> implements Collector {
    ...
    /** 提取这个区间的docID: [start .. start+howMany) 包前不包后
     *  数据在PriorityQueue是按分数由低到高排列的
     *
    ...
    public TopDocs topDocs(int start, int howMany) {
        ...
        // 删除 start+howMany 之后的所有元素,也就是pop 那些分数小的
        for (int i = pq.size() - start - howMany; i > 0; i--) { pq.pop(); }

        // Get the requested results from pq.
        // 从移除元素过后的queue上，由顶向下取howMany个元素。 但存放的数据是倒序的, 也就是越底端的数据放在数组的序号越前
        // 也就是返回的数据 第一位是匹配度最高的。 匹配度递减
