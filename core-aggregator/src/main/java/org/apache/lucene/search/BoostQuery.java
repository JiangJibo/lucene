...
/**
 * A {@link Query} wrapper that allows to give a boost to the wrapped query. Boost values that are less than one will give less importance to this query
 * compared to other ones while values that are greater than one will give more importance to the scores returned by this query.
 * <p>
...
public final class BoostQuery extends Query {
    ...
    /**
     * 如果boost == 0, 那么就是不需要评分, 就返回{@link ConstantScoreQuery} 否则
     */
    private final float boost;

    /**
     * Sole constructor: wrap {@code query} in such a way that the produced scores will be boosted by {@code boost}.
     */
